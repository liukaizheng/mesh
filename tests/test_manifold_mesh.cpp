...
void test_manifold_vertex_halfedges() {
  ...
  const Mesh mesh = Mesh::new_in(
      std::vector<std::vector<std::size_t>>{{0, 2, 3}, {0, 1, 2}, {0, 3, 4}});

  const auto incoming_vertices =
      mesh.vertex(gpf::VertexId{0}).incoming_halfedges() |
      std::views::transform([](const auto he) { return he.from().id; }) |
      std::ranges::to<std::vector>();
  const auto outgoing_vertices =
      mesh.vertex(gpf::VertexId{0}).outgoing_halfedges() |
...
void test_manifold_mesh_tetrahedron_closed() {
  ...
  assert((vertices == std::vector<gpf::VertexId>{gpf::VertexId{2ull},
                                                 gpf::VertexId{3ull},
                                                 gpf::VertexId{1ull}}));
